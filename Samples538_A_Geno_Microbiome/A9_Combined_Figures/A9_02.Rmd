...
volcano_plot_legend <- cowplot::get_legend(A1_01_Samples538_55gene_logistic_additive_volcano_plot)
...
additive_combined <- plot_grid(plot1, plot2,
                               labels = c("A", "B"),
                               label_size = 10,
                               label_y = 1.05,
                              # label_x = -0.02,
...
# Combine the plots (without the legend)
recessive_combined <- plot_grid(plot1, plot2,
                               labels = c("C", "D"),
                               label_size = 10,
                               label_y = 1.05,
                              # label_x = -0.02,
...
## combine additive and recessive model plots

```{r}
# Combine the dominant and recessive plots
additive_resessive_combined <- plot_grid(additive_combined,
                                         recessive_combined,
                               label_size = 10,
                               label_y = 1.05,
                               ncol = 1)
...
additive_resessive_combined_legend <- plot_grid(
  additive_resessive_combined,
  volcano_plot_legend,                  
  nrow = 1,                       
    rel_widths = c(20, 6.5)           
)
...
print(additive_resessive_combined_legend)
...
common_SKAT_legend = cowplot::get_plot_component(A4_01_Common_Rare_w_Weights_SKAT_res,
...
combined_SKAT_plots = plot_grid(plot1,
                                labels = "E",
                                label_size = 10)
...
combined_QQ_volano_SKAT_plot = plot_grid(
                                    additive_resessive_combined_legend + theme(plot.margin = my_plot_margins),
                                    final_SKAT_plot ,
                                    ncol = 1,
                                    rel_heights = c(2.4, 1)
                                    )
