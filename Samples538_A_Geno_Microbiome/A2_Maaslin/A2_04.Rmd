---
title: "Maaslin2 on 16S and ITS with confounders Species detailed result"
...
Plaque538_16S_Species_ps %>% otu_table() %>% rownames() %>% head()
Plaque538_16S_Species_ps %>% tax_table() %>% head()
...
modified_row_names <- replace_chars(rownames(otu_table(Plaque538_16S_Species_ps)))

# Assign back to the phyloseq object
taxa_names(Plaque538_16S_Species_ps) <- modified_row_names
rownames(otu_table(Plaque538_16S_Species_ps)) <- modified_row_names


# Modify taxa names
new_taxa_names_16S_Species <- gsub("s__", "B_s_", taxa_names(Plaque538_16S_Species_ps))

# Apply new names back to the phyloseq object
taxa_names(Plaque538_16S_Species_ps)          <- new_taxa_names_16S_Species
rownames(otu_table(Plaque538_16S_Species_ps)) <- new_taxa_names_16S_Species
...
```{r}
# Apply replace_chars to the row names of the OTU table
modified_row_names <- replace_chars(rownames(otu_table(Plaque538_ITS_Species_ps)))

# Assign back to the phyloseq object
taxa_names(Plaque538_ITS_Species_ps) <- modified_row_names
rownames(otu_table(Plaque538_ITS_Species_ps)) <- modified_row_names


# Modify taxa names
new_taxa_names_ITS_Species <- gsub("s__", "F_s_", taxa_names(Plaque538_ITS_Species_ps))

# Apply new names back to the phyloseq object
taxa_names(Plaque538_ITS_Species_ps)          <- new_taxa_names_ITS_Species
rownames(otu_table(Plaque538_ITS_Species_ps)) <- new_taxa_names_ITS_Species


# Confirm the changes
Plaque538_ITS_Species_ps %>% otu_table() %>% rownames() %>% head()
...
```{r}
Maaslin2_Species_output_dir_16S = here("Samples538_A_Geno_Microbiome/A2_Maaslin",
                           "Maaslin2_16S_Species_output")
# if file error quit sesssion and connect

if (!dir.exists(Maaslin2_Species_output_dir_16S)) {
  dir.create(Maaslin2_Species_output_dir_16S, recursive = TRUE)
}

# Do it for its data
Maaslin2_Species_output_dir_ITS = here("Samples538_A_Geno_Microbiome/A2_Maaslin",
                           "Maaslin2_ITS_Species_output")
# if file error quit sesssion and connect

if (!dir.exists(Maaslin2_Species_output_dir_ITS)) {
  dir.create(Maaslin2_Species_output_dir_ITS, recursive = TRUE)
}
...
#for(var in c("ECC_status", "Child_dental_health")) { # for try
  ...
  Maaslin2_res_Species_16S <- Maaslin2(input_data = t(otu_table(pseq)),
         input_metadata = sample_data(pseq) %>% data.frame(),
         output = Maaslin2_Species_output_dir_16S,
         normalization = "CLR",
         transform = "NONE",
  ...
  Maaslin2_res_Species_16S_df <- Maaslin2_res_Species_16S$results %>% 
                 data.frame() %>% 
                  rename(OTUs = feature ) %>% 
                 filter(metadata  == var) %>% 
  ...
  df_coef_16S <- Maaslin2_res_Species_16S_df %>% dplyr::select(OTUs, coef) %>% 
                     rename(!!var := coef)
                     
  df_qval_16S <- Maaslin2_res_Species_16S_df %>% dplyr::select(OTUs, qval) %>% 
                      rename(!!var := qval)
...
#   Species_coef_16S <- full_join(Species_coef_16S, list_coef_16S[[i]], by = "OTUs")
# }

df_qval_16S <- list_qval_16S[[1]]
# for(i in 2:length(list_qval_16S)) {
#   Species_qval_16S <- full_join(Species_qval_16S, list_qval_16S[[i]], by = "OTUs")
# }
Maaslin2_res_Species_16S
```
...
#for(var in c("ECC_status", "Child_dental_health")) { # for try
  # Run Maaslin2 analysis
  Maaslin2_res_Species_ITS <- Maaslin2(input_data = t(otu_table(pseq)),
         input_metadata = sample_data(pseq) %>% data.frame(),
         output = Maaslin2_Species_output_dir_ITS, # if file error quit sesssion and connect
         normalization = "CLR",
         transform = "NONE",
         fixed_effects =  c(var, confounders),
         cores = 40,
    ...
    # Filter and transform the output data frame
  Maaslin2_res_Species_ITS_df <- Maaslin2_res_Species_ITS$results %>% 
                 data.frame() %>% 
                  rename(OTUs = feature ) %>% 
                 filter(metadata  == var) %>% 
                 dplyr::select(OTUs, coef, qval, stderr)
  ...
  # Rename the coef and qval columns to the current variable
  df_coef_ITS <- Maaslin2_res_Species_ITS_df %>% dplyr::select(OTUs, coef) %>% 
                     rename(!!var := coef)
                     
  df_qval_ITS <- Maaslin2_res_Species_ITS_df %>% dplyr::select(OTUs, qval) %>% 
                      rename(!!var := qval)
...
#   Species_coef_ITS <- full_join(Species_coef_ITS, list_coef_ITS[[i]], by = "OTUs")
# }

df_qval_ITS <- list_qval_ITS[[1]]
# for(i in 2:length(list_qval_ITS)) {
#   Species_qval_ITS <- full_join(Species_qval_ITS, list_qval_ITS[[i]], by = "OTUs")
# }

Maaslin2_res_Species_ITS
```

## combine 16S and ITS

```{r}
Maaslin2_res_Species_16S_ITS = rbind(Maaslin2_res_Species_16S_df, Maaslin2_res_Species_ITS_df) %>% 
  # filter(qval < 0.01) %>%
  dplyr::select(OTUs, coef, qval, stderr)
Maaslin2_res_Species_16S_ITS
```
...
Species_coef_qval_16S_ITS = Maaslin2_res_Species_16S_ITS %>% 
...
Species_coef_qval_16S_ITS

saveRDS(Species_coef_qval_16S_ITS,
...
```{r}
# Data preparation
Species_coef_qval_16S_ITS_2 <- Species_coef_qval_16S_ITS %>% 
  filter(q_values < 0.01) %>% 
  # filter(Coefficients >= 0.5 | Coefficients <= -0.5) %>% 
  identity()

Species_coef_qval_16S_ITS_2$log_q_values <- -log10(Species_coef_qval_16S_ITS_2$q_values)
Species_coef_qval_16S_ITS_2$Sign <- ifelse(Species_coef_qval_16S_ITS_2$Coefficients > 0, "ECC", "CF")


# Use abbreviate to shorten names while avoiding duplicates
Species_coef_qval_16S_ITS_2$Shortened_OTUs <- abbreviate(Species_coef_qval_16S_ITS_2$OTUs, minlength = 48, use.classes = FALSE)
Species_coef_qval_16S_ITS_2$Shortened_OTUs <- factor(Species_coef_qval_16S_ITS_2$Shortened_OTUs, levels = Species_coef_qval_16S_ITS_2$Shortened_OTUs[order(Species_coef_qval_16S_ITS_2$Coefficients)])

Species_coef_qval_16S_ITS_2
```


```{r}
Species_coef_plot=
        ggplot(Species_coef_qval_16S_ITS_2, aes(x = Shortened_OTUs, y = Coefficients, fill = Sign)) +
  ...
  scale_y_continuous(limits = c(-2, 2.5), breaks = seq(-2, 2, 1)) +
        ...
        geom_text(    # Feature labeling
            ...
            hjust = ifelse(Species_coef_qval_16S_ITS_2$Coefficients < 0, 0, 1),
            nudge_y = ifelse(Species_coef_qval_16S_ITS_2$Coefficients < 0, 0.1, -0.1),
            color = "black",
            size = 3) +
        theme(    # Guide lines
...
# Save the plot
 ggsave(here("Samples538_A_Geno_Microbiome/A2_Maaslin",
             "A2_04_Maaslin2_Species_barplot.jpg"),
       plot = Species_coef_plot,
       width = 6, height = 7, dpi = 300)
 
# save the plot as rds
saveRDS(Species_coef_plot,
         file = here("Samples538_A_Geno_Microbiome/A2_Maaslin",
                     "A2_04_Maaslin2_Species_barplot.rds")
         )
```
