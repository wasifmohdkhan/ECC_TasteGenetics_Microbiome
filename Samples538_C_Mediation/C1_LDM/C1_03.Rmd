---
title: "LDM on Samples538 sig_variant_sig_OTU with clr"
...
```{r}
file_name_ext="C1_03_sig_variant_sig_OTU"
```
...
OTUs_16S_ITS_2 = Samples538_covariates_vcf_clr_microbiome %>% 
  ...
  column_to_rownames(var = "IID") %>%
  dplyr::select(all_of(Maaslin2_16S_ITS_features_names)) 
OTUs_16S_ITS_2
```
...
# formula_str <- paste0("OTUs_16S_ITS_2 | (Age+Sex+PC1+PC2+PC3+PC4+PC5+RuralUrban+SEFI_score) ~ ",
#                         snp, " + ECC_status")
...
Samples538_ldm_one_variant_all_OTU_2 <- Samples538_ldm_one_variant_all_OTU %>%
  mutate(log_P_Value = -log10(P_Value)) %>%
  # Convert columns to character to avoid issues with factors
  mutate(OTU_Detected = as.character(OTU_Detected),
         SNP = as.character(SNP)) %>%
  # Add missing SNP rows (if any) for each OTU with NA values
  bind_rows(
    expand_grid(
      OTU_Detected = unique(.$OTU_Detected),
      SNP = setdiff(Samples538_recoded_vcf_sig_names, unique(.$SNP))
    ) %>%
      mutate(P_Value = NA, log_P_Value = NA)
  ) %>%
  # Ensure every OTU has all the sig SNPs, filling missing p-values with 0
  complete( SNP = Samples538_recoded_vcf_sig_names, OTU_Detected,
           fill = list(P_Value = 0, log_P_Value = 0))
Samples538_ldm_one_variant_all_OTU_2
```


```{r}
# Create the heatmap
heatmap_plot <- ggplot(Samples538_ldm_one_variant_all_OTU_2,
                       aes(x = SNP, y = OTU_Detected, fill = log_P_Value )) +
  geom_tile(color = "black", alpha = 0.8) +
  scale_x_discrete(labels = function(labels) str_replace(labels, "chr\\d+[_]+\\d+_", "")) +
  scale_y_discrete(labels = function(labels) str_trunc(labels, width = 36, side = "right")) +
  scale_fill_gradientn(colors = c("white", "red", "darkred"),
                       name = expression("-log10("*italic(p)*"-value)")
                       ) +
  ...
  theme_bw() +
   theme(
    ...
    legend.title = element_text(size = 5),  # Remove legend title for clarity
    legend.text = element_text(size = 4),  # Adjust legend text size
    legend.key.size = unit(0.4, "cm"),  # Adjust the size of the legend key symbols
  ) +
  theme(
    axis.text = element_text(size = 6, color = "black"),
    axis.title = element_text(size = 7, color = "black", face = "bold", margin = margin(r = 10)),
...
ggsave(here("Samples538_C_Mediation/C1_LDM",
                      paste0(file_name_ext, ".jpg")),
       width = 3.8, height = 2.7)
saveRDS(heatmap_plot, here("Samples538_C_Mediation/C1_LDM",
                      paste0(file_name_ext, ".rds")))
