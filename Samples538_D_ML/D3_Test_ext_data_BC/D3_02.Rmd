---
title: "16S ML models comparison and RF vip and perf on ext sPLSDA data"
...
```{r}
# job::job({ rmarkdown::render("/media/Second_stor/wasif/WK_Rprojects/RP_240910_Taste_Genes_Analysis_2/Samples538_D_ML/D3_Test_ext_data_BC/D1_01.Rmd")}, title = "Running Rmd D1_01 in background")
```
...
```
```{r}
Plaque538_16S_Species_CLR_OTU = readRDS(here("Samples538_D_ML/D3_Test_ext_data_BC/sPLSDA_Datasets",
...
```{r}
Plaque538_16S_Species_CLR_OTU_sig = Plaque538_16S_Species_CLR_OTU_2 %>%
  # dplyr::select(one_of(Maaslin2_16S_ITS_features_names)) %>% 
  identity()
Plaque538_16S_Species_CLR_OTU_sig

## feature names
Plaque538_16S_Species_CLR_OTU_sig_names = Plaque538_16S_Species_CLR_OTU_sig %>% 
  dplyr::select(-IID) %>%
  names()
Plaque538_16S_Species_CLR_OTU_sig_names %>% head()
...
Samples538_ml_data <- Samples538_covariates_vcf_clr_microbiome %>%
  ...
  # remove microbiome
  # dplyr::select(-all_of(Plaque538_16S_Species_CLR_OTU_sig_names)) %>%
  identity()
Samples538_ml_data
``` 
...
saveRDS(tuning_results,
        file = here("Samples538_D_ML/D3_Test_ext_data_BC",
            paste0(file_name_ext, "_tuning_results.rds")))
```

```{r}
tuning_results <- readRDS(file = here("Samples538_D_ML/D3_Test_ext_data_BC",
            paste0(file_name_ext, "_tuning_results.rds")))
```
...
saveRDS(auroc_auprc_values,
        here("Samples538_D_ML/D3_Test_ext_data_BC",
...
saveRDS(VIP_df_RF, here("Samples538_D_ML/D3_Test_ext_data_BC",
...
dataset_folder <- here("Samples538_D_ML/D3_Test_ext_data_BC/sPLSDA_Datasets")  # Folder where the external dataset is stored
...
for (model_name in model_names) {
  ...
  for (dataset_name in dataset_names) {
    ...
    dataset_file <- file.path(dataset_folder, paste0(dataset_name, "_MaAsLin2_CLR_sPLSDA_ECCstatus.rds"))
...
saveRDS(final_results, file = here("Samples538_D_ML/D3_Test_ext_data_BC",
                                   paste0(file_name_ext, "_ext_datasets_arroc_auprc_results.rds")
                                   )
        )
...
```{r}
# Read the data
ext_datasets_arroc_auprc_results = readRDS(here("Samples538_D_ML/D3_Test_ext_data_BC",
                                                paste0(file_name_ext, "ext_datasets_arroc_auprc_results.rds")
                                                ))
...
ggsave(here("Samples538_D_ML/D3_Test_ext_data_BC",
            paste0(file_name_ext, "_ext_datasets_AUROC_AUPRC_Comparison.jpg")
             ),
...
# Plot with facets for AUROC and AUPRC, applying a color palette from ggsci
ext_datasets_arroc_auprc_results_plot = 
ggplot(long_results, aes(y = dataset_id, x = Value, fill = Metric, , width=.7)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.8, color = "black") +
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "red", linewidth = 0.2) +
  labs(
...
ggsave(here("Samples538_D_ML/D3_Test_ext_data_BC",
            paste0(file_name_ext, "_ext_datasets_AUROC_AUPRC_Comparison_RF.jpg")),
       ext_datasets_arroc_auprc_results_plot,
       width = 4.5, height = 3.5, dpi = 300)
# save rds
saveRDS(ext_datasets_arroc_auprc_results_plot,
        file = here("Samples538_D_ML/D3_Test_ext_data_BC",
                                  paste0(file_name_ext,
                                         "_ext_datasets_arroc_auprc_results_RF.rds"))
        )
```
