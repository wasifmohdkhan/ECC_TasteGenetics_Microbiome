---
title: "mbQTL, sig_variant_sig_species ECC only"
...
### list of ECC samples
```{r}
ECC_samples = Samples538_covariates %>% 
  filter(ECC_status == 1) %>% 
  dplyr::select(IID) %>% 
  pull()
ECC_samples
...
```{r}
Maaslin2_16S_ITS_features_2_ECC = Maaslin2_16S_ITS_features_2[ECC_samples,]
Maaslin2_16S_ITS_features_2_ECC %>% head()



Samples312_recoded_vcf_2_ECC = Samples538_recoded_vcf_2[ECC_samples, , drop = F]
Samples312_recoded_vcf_2_ECC %>% head()


Samples538_covariates_2_t_ECC = Samples538_covariates_2_t[ ,ECC_samples] %>% 
  subset(., rownames(Samples538_covariates_2_t) != "ECC_status")
Samples538_covariates_2_t_ECC
```


```{r}
Samples312_LinearAnalysisTaxaSNP <- linearTaxaSnp(Maaslin2_16S_ITS_features_2_ECC,
  Samples312_recoded_vcf_2_ECC,
  Samples538_covariates_2_t_ECC
)
Samples312_LinearAnalysisTaxaSNP 
...
```{r}
Samples312_LinearAnalysisTaxaSNP_2 = Samples312_LinearAnalysisTaxaSNP %>%
...
filter(P_BH_adj < 0.05) %>% 
  ...
  select(-FDR)
Samples312_LinearAnalysisTaxaSNP_2 
```


## Save as tsv
```{r}
# write.table(Samples312_LinearAnalysisTaxaSNP_2, 
#            file = here("Samples312_B_MicrbiomeQTL/B1_mbQTL",
#                        "B1_03_Samples312_mbQTL_sig_variant_sig_species.tsv"),
#            quote = F, sep = "\t", row.names = F)

```

```{r}
Samples312_LinearAnalysisTaxaSNP_spread = Samples312_LinearAnalysisTaxaSNP %>% # Samples312_LinearAnalysisTaxaSNP_2
  ...
  replace(is.na(.), 0)
Samples312_LinearAnalysisTaxaSNP_spread %>% head()

Samples312_LinearAnalysisTaxaSNP_gather = Samples312_LinearAnalysisTaxaSNP_spread %>%
  gather(key = "snps", value = "nlog10P", -Genus) 
Samples312_LinearAnalysisTaxaSNP_gather %>% head()

Samples312_LinearAnalysisTaxaSNP_gather$Genus <- abbreviate(Samples312_LinearAnalysisTaxaSNP_gather$Genus, minlength = 35, use.classes = FALSE)
Samples312_LinearAnalysisTaxaSNP_gather %>% head()
```
...
```{r}
# Create the heatmap
Samples312_LinearAnalysisTaxaSNP_heatmap <- 
  ggplot(Samples312_LinearAnalysisTaxaSNP_gather,
  ...
  scale_y_discrete(limits = rev(levels(Samples312_LinearAnalysisTaxaSNP_gather$snps)),
  ...
  labs( y = "Genetic Variants", x = "Microbial Species",
        title = "mbQTL for with ECC_status as covairate")

Samples312_LinearAnalysisTaxaSNP_heatmap
# ggsave(here("Samples538_B_MicrbiomeQTL/B1_mbQTL",
#             "B1_03.2_Samples312_mbQTL_sig_variant_sig_species.png"),
#        Samples312_LinearAnalysisTaxaSNP_heatmap,
#        width = 6,
#        height = 4,
#        dpi = 300)
