---
title: "B4_03_top_variant_all_OTU_ECC lm  "
...
```{r}
file_name_ext="B4_03_top_variant_all_OTU_ECC"
```
...
# To include only ECC samples 
```{r}
#to include only ECC samples
Samples312_covariates_vcf_clr_microbiome_ECC = Samples538_covariates_vcf_clr_microbiome %>% 
  filter(ECC_status == 1)
Samples312_covariates_vcf_clr_microbiome_ECC
```
...
linear_model <- glm(B_s_Streptococcus_mutans ~ chr15_40303364_PLCB2_rs2305645 +Age+Sex+PC1+PC2+PC3+PC4+PC5+RuralUrban+SEFI_score,
                     data =Samples312_covariates_vcf_clr_microbiome_ECC,
                     family = gaussian)
...
ggsave(here("Samples538_B_MicrbiomeQTL/B4_lm",
       ...
       height = 4,
       units = "in")
saveRDS(variant_otu_heatmap, here("Samples538_B_MicrbiomeQTL/B4_lm",
...
#   geom_text(aes(label = significance), vjust = 0.5, color = "black", size = 2) + # Add combined significance markers
#   scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
#                        midpoint = 0, name = "Estimate") +      # Color scale for estimates
#    # Abbreviate y-axis labels on the fly
#   scale_y_discrete(labels = function(labels) str_trunc(labels, width = 35, side = "right")) +
#   scale_x_discrete(labels = function(labels) str_trunc(labels, width = 35, side = "right")) +
...
#                                face = ifelse(linear_model_results_df_3$variant %in% Samples538_Plink_logit_covASPB_w_anno_2_sig,
#                                               "bold", "plain")
#                                ),  
...
#     legend.key.size = unit(0.25, "cm")  ,                           # Smaller legend key
