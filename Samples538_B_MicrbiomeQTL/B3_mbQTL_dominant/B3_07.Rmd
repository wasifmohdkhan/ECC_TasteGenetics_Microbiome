---
title: "mbQTL_sig_variant_sig_species_dominant_model_CF_only"
...
# extract CF sample sampleid
```{r}
CF_sampleid = Samples538_covariates %>% 
...
# new datasets with CF samples only

```{r}
Maaslin2_16S_ITS_features_3 = Maaslin2_16S_ITS_features_2 %>% 
   dplyr::filter(rownames(.) %in% CF_sampleid)
...
Samples538_covariates_3_t = Samples538_covariates_2_t %>% 
  dplyr::select(all_of(CF_sampleid)) %>% 
  # remove the rows containing the CF status
...
ggsave(here("Samples538_B_MicrbiomeQTL/B3_mbQTL_dominant",
            "B3_07_Samples538_mbQTL_sig_variant_sig_species_CF_only.png"),
       Samples538_LinearAnalysisTaxaSNP_heatmap,
       width = 6,
       height = 4,
       dpi = 300)
