---
title: "mbQTL_sig_variant_sig_species_dominant_model_ECC_only"
...
# extract ECC sample sampleid
```{r}
ECC_sampleid = Samples538_covariates %>% 
  dplyr::select(ECC_status, FID) %>% 
  filter(ECC_status == 1) %>% 
  dplyr::select(FID) %>%
  pull()
ECC_sampleid
...
# new datasets with ECC samples only

```{r}
Maaslin2_16S_ITS_features_3 = Maaslin2_16S_ITS_features_2 %>% 
   dplyr::filter(rownames(.) %in% ECC_sampleid)
Maaslin2_16S_ITS_features_3

Samples538_recoded_vcf_3 = Samples538_recoded_vcf_2 %>% 
  dplyr::filter(rownames(.) %in% ECC_sampleid)
Samples538_recoded_vcf_3

Samples538_covariates_3_t = Samples538_covariates_2_t %>% 
  dplyr::select(all_of(ECC_sampleid)) %>% 
  # remove the rows containing the ECC status
  dplyr::filter(rownames(.) != "ECC_status")
Samples538_covariates_3_t
```


```{r}
Samples538_LinearAnalysisTaxaSNP <- linearTaxaSnp(Maaslin2_16S_ITS_features_3,
  Samples538_recoded_vcf_3,
  Samples538_covariates_3_t
)
...
#                        "B3_03_Samples538_mbQTL_sig_variant_sig_species.tsv"),
...
ggsave(here("Samples538_B_MicrbiomeQTL/B3_mbQTL_dominant",
            "B3_06_Samples538_mbQTL_sig_variant_sig_species_ECC_only.png"),
       Samples538_LinearAnalysisTaxaSNP_heatmap,
       width = 6,
       height = 4,
       dpi = 300)
